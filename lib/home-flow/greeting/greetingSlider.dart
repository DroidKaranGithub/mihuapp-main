...
class GallerySliderWidget extends StatefulWidget {
  final bool showEditButttom;
  final post;

  const GallerySliderWidget(
      {super.key, required this.showEditButttom, required this.post});

  @override
  _GallerySliderWidgetState createState() => _GallerySliderWidgetState();
}

class _GallerySliderWidgetState extends State<GallerySliderWidget> {
  int _current = 0;
  ...
  Widget build(BuildContext context) {
    ...
    final String itemUrl = widget.post.photo;
    return Padding(
                              ...
                              Container(
                                ...
                                child: PageView(
                                  ...
                                  children: List.generate(
                                    ...
                                    // final String itemUrl = widget.post;
                                    final bool isVideo =
                                ...
                                child: CarouselButton(
                                  ...
                                  onPressed: () async {
                                    ...
                                    if (itemUrl.endsWith('.mp4') ||
                                        itemUrl.endsWith('.avi') ||
                                        itemUrl.endsWith('.mov')) {
                                      await _downloadVideo(itemUrl,
                                          context); // Call the download video method
                                    } else {
                                      List<Layers> layers = sliderController
                                              .frames[_current].layers ??
                                          [];
                                      final String itemUrl =
                                          widget.post['photo'];
                                      await DownloadHelper.downloadPhoto(layers,
                                          itemUrl); // Call the updated download image method
                                    }
                                  },
                                  icon: Image.asset(
  ...
  Future<void> _downloadVideo(String videoUrl, BuildContext context) async {
    try {
      ...
      String savePath =
          '${tempDir.path}/downloaded_video.mp4'; // Specify the file name
